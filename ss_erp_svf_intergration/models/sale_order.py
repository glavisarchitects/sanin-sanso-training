...
class SaleOrder(models.Model):
    ...
    def estimation_request_svf_template_export(self):
        ...
        due_date = str(self.x_due_date_time) + '日間' if self.x_due_date_time else self.validity_date.strftime(
            "%Y年%m月%d日")
        delivery_note = self.x_delivery_note if self.x_delivery_note else self.x_delivery_date.strftime("%Y年%m月%d日")
        payment_term = self.x_payment_term if self.x_payment_term else self.payment_term_id.name
